...
def full_evaluate(args, corpus, tokenized_dataset):
    ...
    return
#####################################################################################################
# Why are some reply-tos not included in the dataset?

from datasets import Dataset, DatasetDict

def processsingleDialog(dialog):
    """
    We assume this function can only be used with wikiconv dataset
    Given a ConvoKit conversation, preprocess each utterance's text by tokenizing and truncating.
    Returns the processed dialog entry where text has been replaced with a list of
    tokens, each no longer than MAX_LENGTH - 1 (to leave space for the EOS token)
    """
    ...
    for utterance in dialog.iter_utterances():
        ...
        if utterance.meta['is_section_header']:
            continue
        is_attack = int(utterance.meta[utt_label_metadata]) 
        reply_to = utterance.reply_to
        processed_list.append({"is_attack": is_attack, "id": utterance.id, "reply_to": reply_to})
        processed_dict[utterance.id] = {"text": utterance.text, "is_attack": is_attack, "id": utterance.id}
    return processed_dict, processed_list

def corpus2singledataset(corpus, split=None, last_only=False, shuffle=False):
    """
    This function is used for training and validating during training only.
    Load context-reply pairs from the Corpus, optionally filtering to only conversations
    from the specified split (train, val, or test).
    Each conversation, which has N comments (not including the section header) will
    get converted into N-1 comment-reply pairs, one pair for each reply 
    ...
    for convo in corpus.iter_conversations():
        # consider only conversations in the specified split of the data
        if split is None or convo.meta['split'] == split:
            dialog_dict, dialog_list = processsingleDialog(convo)
            trigger_id = None
            for utterance in dialog_list:
                if utterance["is_attack"]:
                    trigger_id = utterance["reply_to"]
                    label = 1
                    break
            if trigger_id == None:
                trigger_id = utterance["reply_to"]
                label = 0
            try:
                context = [dialog_dict[trigger_id]["text"]]
                dataset_dict["context"].append(context)
                dataset_dict["id"].append(trigger_id)
                dataset_dict["labels"].append(label)
                added += 1
            except:
                omitted += 1
